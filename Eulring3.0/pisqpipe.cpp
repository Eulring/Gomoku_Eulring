...
int terminated; /* return from brain_turn when terminate>0 */
...
void makestrlog(const char *str) {
	char buffer[200][33];
	int cnt = 0;
	ifstream infile("log.txt");
	while (!infile.eof()) {
		infile.getline(buffer[cnt], 20);
		cnt++;
		if (strlen(buffer[cnt - 1]) <= 1) cnt--;
	}
	infile.close();
...
static void do_command()
{
	...
	if ((param = get_cmd_param("info", cmd)) != 0) {
		...
		if ((info = get_cmd_param("timeout_turn", param)) != 0) {
			...
			brain_info(0, info_timeout_turn);
		}
	...
	else if ((param = get_cmd_param("print", cmd)) != 0){
		brain_print();
	}
	else if ((param = get_cmd_param("debug", cmd)) != 0){
		brain_debug();
	}
